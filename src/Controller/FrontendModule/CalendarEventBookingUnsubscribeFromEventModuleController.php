            ...
            if ('true' !== $request->query->get('has_unsubscribed')) {
                ...
                if (!$this->hasError) {
                    ...
                    if (!empty($this->event->unsubscribeLimitTstamp)) {
                        ...
                        if (strtotime('now') > $this->event->unsubscribeLimitTstamp) {
                        ...
                        if ($this->event->addTime && $this->event->startTime > $this->event->startDate) {
                            if (strtotime('now') + $limit * 3600 * 24 > $this->event->startTime) {
                                $blnLimitExpired = true;
                            }
                        } else {
                            if (strtotime('today 00:00') + $limit * 3600 * 24 > $this->event->startDate) {
                                $blnLimitExpired = true;
                            }
                        }
                    }
