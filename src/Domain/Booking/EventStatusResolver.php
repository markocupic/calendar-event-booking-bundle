...
{
    public const string DRAFT = 'draft'; // Event is being created but not yet visible or bookable.

    public const string BOOKING_OPEN = 'booking_open'; // Users can register or buy tickets.

    public const string FULLY_BOOKED = 'fully_booked'; // All spots are taken; booking no more allowed.

    public const string WAITING_LIST_OPEN = 'waiting_list_open'; // Event is full, but users can join a waiting list.

    public const string NOT_BOOKABLE = 'not_bookable'; // Event is not bookable.

    public const string NOT_YET_BOOKABLE = 'not_yet_bookable'; // Event is not yet bookable. Waiting for the booking period to start.

    public const string BOOKING_CLOSED = 'booking_closed'; // Booking period has ended (manually or automatically).
     ...
     */
    public function resolveEventStatus(CalendarEventsModel $calEvent, Request $request): string
    {
        $eventStatus = $this->determineEventStatus($calEvent);
        $event = new ResolveEventStatusEvent($calEvent, $request, $eventStatus);
    ...
    protected function determineEventStatus(CalendarEventsModel $calEvent): string
    {
        ...
        if ('' !== (string) $calEvent->start && $calEvent->start > time()) {
            return self::DRAFT;
        }

        // Is the event no more published?
        if ('' !== (string) $calEvent->end && $calEvent->end < time()) {
            return self::DRAFT;
        }

        // Is booking disabled?
        if (!$calEvent->enableBookingForm) {
            return self::NOT_BOOKABLE;
        }

        // Is the event not started yet?
        if ($calEvent->bookingStartDate > time()) {
            return self::NOT_YET_BOOKABLE;
        }

        // Has the booking period ended?
        if (is_numeric($calEvent->bookingEndDate) && $calEvent->bookingEndDate < time()) {
            return self::BOOKING_CLOSED;
        }

        if ($this->bookingCapacity->isFullyBooked($calEvent)) {
            if ($this->bookingCapacity->canFulfillBookingRequestWaitingList($calEvent, 1)) {
